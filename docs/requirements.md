...
| **NovelDraft** | **6 步向导草稿表（完成才转为 NovelProject）** | user_id, inspiration(text), title, genre, current_step(0-6), is_completed, converted_project_id |
...
| NovelSetting | 小说设定表 | project_id, setting_type(worldview/characters/map/storyline/plot_arc/opening), title, content, structured_data(JSON), ai_generated, order |
...
NovelDraft: created → (6步 save-step) → completed (转为 NovelProject)
...
- `POST /api/ai/generate/setting` - 生成小说设定（非流式，6 种类型）
...
- **Home 模式**: 渐变 Banner + 左侧数据概览（总书数/章节/字数）+ 新建书目（灵感输入 → 建档 → 6步向导）+ 我的书目（3列网格卡片）
- **Workspace 模式**: 三栏布局（左侧书库树 + 中央写作控制台 + 右侧知识图谱）

**新书设置向导**: 6步 AI 流水线 Modal 向导（**草稿模式**）
- 流程: 输入灵感 → `POST /api/drafts/` 创建草稿（**不建项目**） → 进入 6 步 Wizard
- 步骤: 世界观 → 人物 → 地图 → 故事线 → 情节弧 → 开始 → 进入工作台
- 前 6 步：进入每步时**自动触发 WebSocket 流式生成**（连接 FastAPI `/ws/generate-setting`），实时逐字显示
- **世界观步骤**: 8 维度卡片垂直排列，每张卡片对应一个维度（时间/地点/社会/文化/科技/力量/历史/自然法则），可逐一编辑
- **地图步骤**: ECharts 力导向图可视化地区关系（靛蓝底色区块）+ 结构化编辑列表（白色底色区块）
- **情节弧步骤**: 垂直 Steps 步骤条展示各幕（起承转合）
- **角色步骤**: 角色卡片垂直列表（名字 + 角色 Tag + 简介）+ ECharts 力导向角色关系图（按阵营着色、按角色类型定大小）
- 用户审核/编辑 AI 输出后，点"下一步"保存到 `DraftSetting` 并前进
- 前序步骤已保存的内容作为上下文传递给后续步骤的 AI 生成
- 第 7 步"进入工作台"：展示所有 6 步设定总览，确认后调用 `POST /api/drafts/{id}/complete/` 创建 NovelProject
